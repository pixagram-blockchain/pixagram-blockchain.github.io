...
class ChipInput extends React.PureComponent {
    ...
    handleKeyDown = (event) => {
        ...
        switch (event.keyCode) {
            ...
            case keyCodes.RIGHT_ARROW:
                ...
                break
            default:
                // Only clear when a chip actually is focused — the old
                // unconditional setState ran a full state merge + sCU pass on
                // EVERY keystroke while typing (focusedChip was already null).
                if (focusedChip != null) {
                    this.setState({ focusedChip: null })
                }
                break
    ...
    /**
     * Per-index chip handler cache. render() previously allocated two fresh
     * closures per chip on every render — i.e. on every keystroke. The cached
     * pair dereferences the live chip list at call time, so it stays correct
     * across adds/deletes/reorders while keeping a stable identity per slot.
     */
    _getChipHandlers (i) {
        const cache = this._chipHandlers || (this._chipHandlers = [])
        if (!cache[i]) {
            cache[i] = {
                handleClick: () => this.setState({ focusedChip: i }),
                handleDelete: () => {
                    const chips = this.props.value || this.state.chips
                    this.handleDeleteChip(chips[i], i)
                }
            }
        }
        return cache[i]
    ...
    render () {
        ...
        const chipComponents = chips.map((chip, i) => {
            ...
            const handlers = this._getChipHandlers(i)
            return chipRenderer(
                {
                    value,
        ...
        InputMore.endAdornment = endAdornment
...
// The four style combinations are constants — the renderer previously built a
// fresh style object per chip per render, nearly always `{ undefined, undefined }`.
const CHIP_STYLE_NONE = undefined
const CHIP_STYLE_LOCKED = { pointerEvents: 'none' }
const CHIP_STYLE_FOCUSED = { backgroundColor: 'rgb(96, 96, 96)' }
const CHIP_STYLE_LOCKED_FOCUSED = { pointerEvents: 'none', backgroundColor: 'rgb(96, 96, 96)' }
...
export const defaultChipRenderer = ({ value, text, isFocused, isDisabled, isReadOnly, handleClick, handleDelete, className }, key) => {
    ...
    return (
        <Chip
            ...
            className={className}
            style={locked
                ? (isFocused ? CHIP_STYLE_LOCKED_FOCUSED : CHIP_STYLE_LOCKED)
                : (isFocused ? CHIP_STYLE_FOCUSED : CHIP_STYLE_NONE)}
            onClick={handleClick}
