...
class PixaWalletDelegateDialog extends React.PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            _isDragging: false,
            _tempPercent: 0,
            _lockedDelegatee: props.lockedDelegatee || false,
        };
    ...
    componentWillReceiveProps(new_props) {
        ...
        // Also reset on open (catches stale _confirm_success from race condition)
        if (new_props.open && !wasOpen) {
            new_props = {
                ...new_props,
                _confirm_open: false,
            ...
            // Prefill delegatee when provided (e.g. viewing someone else's wallet)
            if (new_props.initialDelegatee) {
                new_props._username = new_props.initialDelegatee;
                setTimeout(() => { this._resolveUsername(new_props.initialDelegatee); }, 0);
            }
        }
        const updates = {...new_props};
        if (new_props.maxPXP !== undefined) updates._maxPXP = new_props.maxPXP;
        if (new_props.lockedDelegatee !== undefined) updates._lockedDelegatee = new_props.lockedDelegatee;
    ...
    _confirm_transaction = async () => {
        ...
        const { _username, _amount } = this.state;
        this.setState({_confirm_success: 1, _confirm_error: ""}, () => this.forceUpdate());
        try {
            if (typeof this.props.onDelegate === 'function') {
                await this.props.onDelegate(_username, _amount);
            }
            this.setState({_confirm_success: 2}, () => this.forceUpdate());
        } catch (err) {
            this.setState({_confirm_success: 0, _confirm_error: err.message || t("components.pixa_wallet_delegate_dialog.delegation_failed")}, () => this.forceUpdate());
        }
    };
    ...
    render() {
        const {
            ...
            open, _isDragging, _tempPercent, _lockedDelegatee,
        } = this.state;
        ...
        return (
            <React.Fragment>
                <Dialog open={open}
                    ...
                    <DialogContent>
                        <Typography component={"h2"} variant={"h6"}>{t("components.pixa_wallet_delegate_dialog.delegate")}</Typography>
                        <Typography variant="body2" color="textSecondary" component="p" style={{margin: "16px 0 8px"}}>
                            {t(
                                "components.pixa_wallet_delegate_dialog.delegate_your_pixa_power_pxp_to_another"
                            )}
                        </Typography>
                        <div>
                            <Autocomplete
                                ...
                                renderInput={(params) => (
                                    <TextField
                                        {...params}
                                        label={t("components.pixa_wallet_delegate_dialog.beneficiary")}
                                        variant="filled"
                                        fullWidth
                    ...
                    <DialogActions style={{textAlign: "right"}} className={classes.darkGreyActions}>
                        ...
                        <Fade in={_confirm_success === 0} timeout={450}>
                            <Tooltip title={max <= 0 ? t("components.pixa_wallet_dialog.you_dont_have_any_pxp_to_2") : ""} disableHoverListener={max > 0} disableFocusListener={max > 0} disableTouchListener={max > 0}>
                                <span><Button variant="contained" color="primary" autoFocus onClick={this._open_confirm_dialog} disabled={max <= 0 || !_username || displayAmount <= 0}>{t("words.confirm", {TUC: true})}</Button></span>
                            </Tooltip>
                        </Fade>
                    </DialogActions>
                </Dialog>
                {/* Confirmation dialog (white, like Send) */}
                <Dialog PaperProps={{classes: {root: classes.whiteDialog}}}
                        ...
                        <Collapse in={Boolean(!_confirm_locked)}>
                            <div style={{textAlign: _confirm_success < 2 ? "left": "center"}}>
                                {this.state._confirm_error ?
                                    ...
                                    : _confirm_success < 1 ?
                                        <Typography variant="body2" color="textSecondary" component="p" style={{margin: "4px 0px 8px 0px"}}>{t(
                                            "components.pixa_wallet_delegate_dialog.delegations_can_be_revoked_at_any_time"
                                        )}</Typography>
                                        : _confirm_success < 2 ?
                                            <Typography variant="body2" color="textSecondary" component="p" style={{margin: "4px 0px 8px 0px"}}>{t("components.pixa_wallet_delegate_dialog.the_delegation_is_being_processed")}</Typography>
                                            :<Typography variant="body2" color="textSecondary" component="p" style={{margin: "4px 0px 8px 0px"}}>{t("components.pixa_wallet_delegate_dialog.delegation_completed")}</Typography>
                                }
                            </div>
                        </Collapse>
                    </DialogContent>
                    ...
                    {_confirm_success === 0 &&
                        <DialogActions style={{textAlign: "right"}}>
                            ...
                            <Fade in timeout={450}>
                                <Button variant="contained" color="primary" onClick={() => this._confirm_transaction()} disabled={_confirm_locked}>{t("words.delegate", {TUC: true})}</Button>
                            </Fade>
                        </DialogActions>}
                </Dialog>
            </React.Fragment>
...
export default withLanguage(withStyles(styles)(PixaWalletDelegateDialog));