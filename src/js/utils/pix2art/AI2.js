class HuggingFaceAPI {
    constructor(baseUrl, msgCallback, progressCallback, name) {
    ...
    message(line) {
        try {
            ...
            const previous_date = this.date_message || Date.now() - 100;

            if (previous_date + 100 <= Date.now()) {
                ...
                switch (message) {
                    case "estimation":
                        ...
                        const now = Date.now();
                        this.progressCallback(this.name, now, now + parseInt(rank_eta) * 1000);
                    ...
                    case "process_completed":
                        let string3 = `Processing completed.`
                        const success = data.success;
                        const output = data.output || {};
                        string3 = !success ? output.error : string3;
                        this.msgCallback(string3);
                        break;
                }
            }
    ...
    async readResponse(reader) {
        ...
        while (!done) {
            ...
            for (let i = 0; i < lines.length; i ++) {
                ...
                this.message(line);
                if (line.includes("complete") && line.includes("success\":true") && !line.includes("You have exceeded your GPU quota") && !line.includes("custom_lora_card") && !line.includes("data\":[]")) {
                    done = true;
                    finalLine = line;
        ...
        return Promise.reject("You have exceeded your GPU quota.");
    }
}
export class LongCaptionerAPI extends HuggingFaceAPI {
    constructor(msgCallback, progressCallback) {
        super("https://gokaygokay-sd3-long-captioner-v2.hf.space", msgCallback, progressCallback, "captioner");
    }
...
export class FloranceCaptionerAPI extends HuggingFaceAPI {
    constructor(msgCallback, progressCallback) {
        super("https://primerz-florence-2.hf.space", msgCallback, progressCallback, "captioner");
    }
...
export class RemoveBackgroundAPI extends HuggingFaceAPI {
    constructor(msgCallback, progressCallback) {
        super("https://kenjiedec-rembg.hf.space", msgCallback, progressCallback, "Background remover");
    }
...
export class ImageCreatorAPI extends HuggingFaceAPI {
    constructor(msgCallback, progressCallback) {
        super("https://pixart-alpha-pixart-sigma.hf.space", msgCallback, progressCallback, "Image creator");
    }
    ...
    async run(prompt, width = 512, height = 512, number = 1, output = "imagedata") {
        ...
        if(event_id) {
            ...
            const response = await this.fetchEventSource(this.getResultUrl(hash));
            return this.readResponse(response).then( async (line) => {
...
export const pixelart_style = {
    lucasArt: {
        url: "https://huggingface.co/primerz/lucasart",
        weight: 1.2,
        face: 0.95,
        image: 0.25,
        scale: 9.5,
        strength: 0.9,
        prompt: "In lucasarts style: a beautiful illustration of ('${prompt}':0.75) (in lucasarts style). Artwork, detailed digital painting, bold-edge, sexy character, well aligned face, 2D, illustration, palette-based, natural color. lucasarts style.",
        negative: "Photography, bad quality, ugly, text, bad, bad picture, text, error, ugly, extra fingers, extra toes, mutated hands, mutation, deformed, bad anatomy, bad proportions, extra limbs, disfigured, missing arms, missing legs, extra arms, extra legs, fused fingers, fused toes, too many fingers, too many toes, username, watermark, signature."
    },
    lucasArt2: {
        url: "https://civitai.com/models/151539/lucasarts-style-1990s-pc-adventure-games-sdxl-lora-dreambooth-trained",
        weight: 1.2,
        face: 0.95,
        image: 0.12,
        scale: 5.0,
        strength: 0.8,
        prompt: "An 2D beautiful illustration of ('${prompt}':0.618) in lcas artstyle. Artwork, detailed digital painting, bold-edge, sexy character, 2D, illustration, palette-based, natural color. lcas artstyle.",
        negative: "Photography, bad quality, ugly, text, bad, bad picture, text, error, ugly, extra fingers, extra toes, mutated hands, mutation, deformed, blurry, dehydrated, bad anatomy, bad proportions, extra limbs, disfigured, missing arms, missing legs, extra arms, extra legs, fused fingers, fused toes, too many fingers, too many toes, username, watermark, signature."
    },
    pixa: {
        url: "https://huggingface.co/primerz/Pixa2",
        weight: 3.5,
        face: 0.95,
        image: 0.35,
        scale: 5.0,
        strength: 0.85,
        prompt: "In the style of P1X4... ('${prompt}':0.314)",
        negative: "Photography, bad quality, ugly, text, bad, bad picture, text, error, ugly, extra fingers, extra toes, mutated hands, mutation, deformed, blurry, dehydrated, bad anatomy, bad proportions, extra limbs, disfigured, missing arms, missing legs, extra arms, extra legs, fused fingers, fused toes, too many fingers, too many toes, username, watermark, signature."
    },
    thimbled: {
        ...
        scale: 9.0,
        strength: 0.80,
        prompt: "An high resolution illustration of '${prompt}' in lucasarts style with clean and bold edges. Truthful and beautifully aligned face, very high quality.",
...
export class FaceToAllAPI extends HuggingFaceAPI {
    constructor(msgCallback, progressCallback, num = "777") {
        super("https://primerz-face-to-all-"+num+".hf.space", msgCallback, progressCallback, "Transform");
    ...
    async run(input, prompt, style = "lucasArt") {
        ...
        if (ok && imagePath) {
            ...
            if (typeof event_id !== "undefined") {
                ...
                return this.readResponse(event).then(async (line) => {
...
export class FaceToAllAPI2 extends HuggingFaceAPI {
    constructor(msgCallback, progressCallback, num = "777") {
        super("https://primerz-face-to-all-"+num+".hf.space", msgCallback, progressCallback, "Transform");
        this.styles = pixelart_style;
    }
...
export class FaceToAllAPI3 extends HuggingFaceAPI {
    constructor(msgCallback, progressCallback, num = "777") {
        super("https://primerz-face-to-all-"+num+".hf.space", msgCallback, progressCallback, "Transform");
        this.styles = pixelart_style;
    }
