...
(function(){
    function RgbQuant(opts) {
        ...
        this.initDist = opts.initDist || 0.05;
        // subsequent passes threshold
        this.distIncr = opts.distIncr || 0.005;
        // palette grouping
        this.hueGroups = opts.hueGroups || 16;
        this.lumGroups = opts.lumGroups || 16;
        this.satGroups = opts.satGroups || 16;
        // if > 0, enables hues stats and min-color retention per group
        this.minHueCols = opts.minHueCols || 8;
        // HueStats instance
...
//			buf8 = data.buf8,
    ...
    RgbQuant.prototype.colorStats1D = function colorStats1D(buf32) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            // Existing hue stats (if used)
            if (this.hueStats)
                this.hueStats.check(col);

            // Boost factor based on the user’s chosen percentage (0..1000)
            // E.g., if user sets 200 => 2x weight, 500 => 5x weight, etc.
            var boostFactor = 1;
            if (isSkinTone(r, g, b)) {
                // Convert 0..1000% into a multiplier: e.g. 200 -> 2.0
                boostFactor += this.skinTonePreservation / 100;
            }

            // Add to histogram
            if (histG[col] == null) {
                histG[col] = boostFactor;
            } else {
                histG[col] += boostFactor;
            }
        }
    ...
    function isSkinTone(r, g, b) {
        // Convert to YCbCr
        // There are many formulas; this is a commonly used approximate
        const y  =  0.299*r + 0.587*g + 0.114*b;
        const cb = -0.169*r - 0.331*g + 0.500*b + 128;
        const cr =  0.500*r - 0.419*g - 0.081*b + 128;

        // Rough “skin” thresholds (very approximate!)
        // Real detection often uses narrower / refined ranges
    ...
    function distEuclidean(rgb0, rgb1) {
        var rd = rgb1[0]-rgb0[0]|0,
            gd = rgb1[1]-rgb0[1]|0,
            bd = rgb1[2]-rgb0[2]|0;

        return Math.sqrt(Pr*Math.imul(rd,rd) + Pg*Math.imul(gd,gd) + Pb*Math.imul(bd,bd)) / euclMax;
    }

    var manhMax = Pr*rd + Pg*gd + Pb*bd;
    // perceptual Manhattan color distance
    function distManhattan(rgb0, rgb1) {
        var rd = Math.abs(rgb1[0]-rgb0[0]|0)|0,
            gd = Math.abs(rgb1[1]-rgb0[1]|0)|0,
            bd = Math.abs(rgb1[2]-rgb0[2]|0)|0;
